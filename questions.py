...
ANSWERS = {
    "explicitly_permitted": "an affirmative clause allows it for this tier with no approval or notice step, either naming the activity (express) or permitting dealing where the definitions of dealing and securities reach the activity (via_definition)",
    "not_mentioned_implicitly_permitted": "the tier has no affirmative permission clause and no restriction that reaches the activity, so its only constraint is the law; the mechanism must be none",
    ...
    for r in rows:
        ...
        try:
            ...
            for t in q.tiers:
                ...
                for x in t.findings:
                    want = {"none"} if x.answer == "not_mentioned_implicitly_permitted" else {"express", "via_definition"}
