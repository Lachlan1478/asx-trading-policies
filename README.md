...
| `data/tables/policies.csv` | company | Dates, tiers, window model, definition flags, how far the financing rules reach, the loan-leg answer for senior tiers (banned wide, banned margin only, clearance, notification, permitted, not addressed), forced-sale treatment, disclosure triggers, funded-collar status, summary |

The `funded_collar` column is computed from the rules for the senior tiers (director, KMP, restricted persons): `blocked_hedge` when every senior tier is barred from hedging or OTC derivatives over vested unrestricted shares; `blocked_loan` when secured financing is banned in terms wider than margin loans; `loan_margin_features_banned` when only margin lending is banned, so the loan must avoid margin-call and LVR features; `enforcement_restricted` when a lender's forced sale in a closed period would be a breach; `clearance_required` when such a sale needs clearance; `unclear` when the policy addresses neither hedging nor derivatives and its definition of dealing does not reach derivatives; otherwise `workable`, meaning nothing in the policy blocks it, though clearance and window timing still apply (see `collar.csv`).
